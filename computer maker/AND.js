function AND(input){
    ...
    this.portPos = {
        input : [createVector(this.modulePos.x-10, this.modulePos.y)],
        output : createVector(this.modulePos.x+20, this.modulePos.y)
    }

    this.update = function(){
        this.display()
        if(this.connection.length > 1 && this.input){
            this.input = [this.connection[0].send, this.connection[1].send]
        }
    }
    ...
    this.display = function(){
        ...
        text('A', this.modulePos.x, this.modulePos.y);
    }
